...
class WizardManager extends Component
{
    ...
    protected function syncLinkFieldValue()
    {
        ...
        foreach ($this->wizardConfig['steps'] as $stepIndex => $step) {
            if (empty($step['requiresLink']) || empty($step['model']))
                continue;
    ...
    public function getConfirmationData()
    {
        ...
        foreach ($this->wizardConfig['steps'] as $stepIndex => $step) {
            ...
            if (!$modelAlias || !isset($this->formData[$modelAlias]))
                continue;
            ...
            foreach ($this->formData[$modelAlias] as $field => $value) {
                ...
                if (!empty($step['requiresLink'])) {
                    if (
                        ($this->linkDatabaseFieldName && $field === $this->linkDatabaseFieldName) ||
    ...
    protected function validateCurrentStep()
    {
        ...
        foreach ($rules as $field => $rule) {
            ...
            $prefixedRules["{$modelAlias}.{$cleanField}"] = $rule;
        }
        ...
        $validator = Validator::make($this->formData, $prefixedRules, $messages);
